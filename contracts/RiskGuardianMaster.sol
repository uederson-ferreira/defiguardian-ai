...
contract RiskGuardianMaster is AutomationCompatible, Ownable, ReentrancyGuard {
    ...
    function checkUpkeep(
        ...
        // Estruturas para armazenar execuções necessárias
        address[] memory stopLossUsers = new address[](0);
        uint256[] memory stopLossOrderIds = new uint256[](0);
        address[] memory rebalanceUsers = new address[](0);
        address[] memory volatilityUsers = new address[](0);
        uint256[] memory volatilityPositionIds = new uint256[](0);
    ...
    function _executeStopLossHedges() internal returns (uint256 executions) {
        ...
        // Em implementação real, iterar sobre usuários e suas ordens
        for (uint256 i = 0; i < activeUsers.length && gasRemaining > gasPerExecution; i++) {
            address user = activeUsers[i];
            
            try IStopLossHedge(stopLossHedgeContract).getUserActiveOrders(user) returns (uint256 orderCount) {
