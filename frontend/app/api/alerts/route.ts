...
export async function GET(request: NextRequest) {
  try {
    ...
    const alertStats = {
      ...
      unread:
        statsData?.filter((alert: StatsRecord) => !alert.is_read).length || 0,
      critical:
        statsData?.filter((alert: StatsRecord) => alert.severity === "critical")
          .length || 0,
      high:
        statsData?.filter((alert: StatsRecord) => alert.severity === "high")
          .length || 0,
      medium:
        statsData?.filter((alert: StatsRecord) => alert.severity === "medium")
          .length || 0,
      low:
        statsData?.filter((alert: StatsRecord) => alert.severity === "low")
          .length || 0,
    };
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { portfolioId, alertType, severity, title, message } =
      await request.json();
