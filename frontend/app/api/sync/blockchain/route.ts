    ...
    try {
      ...
      if (CONTRACTS.PORTFOLIO_ANALYZER) {
        ...
        try {
          ...
          const portfolioData = await portfolioContract.getPortfolioData(
            walletAddress
          );
          ...
          if (portfolioId) {
            const { data: updatedPortfolio, error: portfolioError } =
              await supabase
                .from("portfolios")
                .update({
            ...
            if (portfolioError) {
              syncResults.errors.push(
                `Error updating portfolio: ${portfolioError.message}`
              );
            } else {
          ...
          console.error(
            "❌ Error querying Portfolio Analyzer contract:",
            contractError
          );
      ...
      try {
        ...
        if (portfolioId) {
          ...
          if (positionsError) {
            syncResults.errors.push(
              `Error synchronizing positions: ${positionsError.message}`
            );
          } else {
      ...
      try {
        if (
          syncResults.portfolio &&
          Number(syncResults.portfolio.risk_score) > 80
        ) {
          ...
          if (alertError) {
            syncResults.errors.push(
              `Error creating alert: ${alertError.message}`
            );
          } else {
      ...
      console.error(
        "❌ Erro geral na sincronização blockchain:",
        blockchainError
      );
        ...
        if (address && ethers.isAddress(address)) {
          try {
            ...
            contractStatus.contracts[
              name.toLowerCase() as keyof typeof contractStatus.contracts
